...
def index():
    ...
    info = db.execute("SELECT stock, SUM(shares) AS shares, SUM(total) AS total FROM history WHERE id = :id GROUP BY stock", id=session["user_id"])
...
def quote():
    ...
    if request.method == "POST":
        ...
        # Ensure symbol was inserted
        if not request.form.get("symbol"):
            return apology("must provide symbol of the stock",403)

        # find infos about the quote
        quote = lookup(request.form.get("symbol"))

        if quote == None:
            return apology("invalid symbol of the stock", 403)
...
def sell():
    ...
    if request.method == "POST":
        ...
        hist = db.execute("SELECT stock, SUM(shares) AS shares FROM history WHERE id =:id and stock =:name",id=session["user_id"], name= request.form.get("symbol"))
